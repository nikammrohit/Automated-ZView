...
def capture_error_percentage(capture_coords):
    try:
        ...
        threshold_img = cv2.adaptiveThreshold(contrast_img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11,
                                              2)
...
def main():
    ...
    num_iterations = int(
        input("Process A: Enter the number of iterations (rounds) to cycle capture error percentages: "))
    ...
    main_window_x, main_window_y = get_top_left_corner(
        "Please mark the top-left corner of the main ZView 4 window with key press 's'")
    ...
    try:
        ...
        data_window_x, data_window_y = get_top_left_corner(
            "Please mark the top-left corner of the data import window with key press 's'")
